    ...
    <div className="nge-graph-wrap">
      <svg ref={svgRef} className="nge-graph" width="100%" height="100%" viewBox={`0 0 ${width} ${height}`} aria-label="Node graph"
        ...
        <g className={`nge-viewport${transform.animate ? ' is-animated' : ''}`} transform={`translate(${transform.x} ${transform.y}) scale(${transform.k})`}>
          {edges.map((edge) => { const config = relationshipConfig[edge.type]; const active = selectedEdgeId === edge.id || selectedNodeId === edge.source.id || selectedNodeId === edge.target.id; const dim = Boolean(selectedNodeId) && !active; const color = config?.color ?? palette[hash(edge.type) % palette.length]; const sharedProps = { key: edge.id, role: 'button' as const, tabIndex: 0, 'aria-label': `${edge.type}: ${edge.source.label} to ${edge.target.label}`, className: `nge-edge${active ? ' is-active' : ''}${dim ? ' is-dimmed' : ''}`, stroke: color, strokeDasharray: config?.style === 'dashed' ? '7 5' : undefined, markerEnd: (edge.directional ?? config?.directional) ? `url(#nge-arrow-${hash(edge.type)})` : undefined, onClick: (event: ReactMouseEvent) => { event.stopPropagation(); setSelectedNodeId(null); setSelectedEdgeId(edge.id); }, onKeyDown: (event: KeyboardEvent) => { if (event.key === 'Enter' || event.key === ' ') { event.preventDefault(); setSelectedNodeId(null); setSelectedEdgeId(edge.id); } } }; if (edge.source.id === edge.target.id) { const r = 9 + Math.min(9, Math.sqrt(degrees.get(edge.source.id) ?? 0) * 3) + 12; return <path {...sharedProps} d={`M ${edge.source.x},${edge.source.y - (r - 12)} A ${r},${r} 0 1,1 ${edge.source.x + 0.01},${edge.source.y - (r - 12)}`} fill="none" />; } return <line {...sharedProps} x1={edge.source.x} y1={edge.source.y} x2={edge.target.x} y2={edge.target.y} />; })}
